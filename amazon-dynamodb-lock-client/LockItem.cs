...
namespace Amazon.DynamoDBv2
{
    public class LockItem : IDisposable
    {
        ...
        /// <summary>
        /// Only called by mocking framework in unit tests
        /// </summary>
        internal LockItem()
        { }
        ...
        public bool IsExpired()
        {
            ...
            long elapsed = LockClientUtils.MillisecondTime() - this.LookupTime;
            Debug.WriteLine($"ELAPSED: {elapsed} - DURATION: {this.LeaseDuration}");
            return elapsed > this.LeaseDuration;
        }
        ...
        public void Ensure(long leaseDurationToEnsure, TimeUnit timeUnit)
        {
            ...
            {
                this.Client?.SendHeartbeat(new SendHeartbeatOptions(this) { LeaseDurationToEnsure = leaseDurationToEnsure, TimeUnit = timeUnit });
            }
        }
        ...
        /// Marked virtual for unit test requirements
        public virtual void UpdateLookupTime(long lastUpdateOfLock)
        {
        ...
        {
            return $"LockItem{{Partition Key={this.PartitionKey}, Sort Key={this.SortKey}, Owner Name={this.OwnerName}, Lookup Time={this.LookupTime}, Lease Duration={this.LeaseDuration}, Record Version Number={this.RecordVersionNumber}, Delete On Close={this.DeleteLockItemOnclose.ToString().ToLower()}, Is Released={this.IsReleased().ToString().ToLower()}}}";
        }
        ...
        /// Marked virtual for unit test requirements
        internal virtual void UpdateRecordVersionNumber(string recordVersionNumber, long lastUpdateOfLock, long leaseDurationToEnsureInMilliseconds)
        {
        ...
        {
            return this.SessionMonitor != null;
        }
        ...
        internal bool HasCallback()
        {
            ...
            return this.SessionMonitor.HasCallback();
        }

        /// <summary>
        /// Calls RunCallback() on the SessionMonitor object after checking the SessionMonitor's existence.
        /// </summary>
        /// <returns>The running task if a callback on the session monitor was present or null if there was no callback set</returns>
        internal Task RunSessionMonitor()
        {
            if (this.SessionMonitor == null)
            {
            ...
            return this.SessionMonitor.RunCallBack();
        }
        ...
        {
            return $"{this.PartitionKey}{this.SortKey}";
        }
        ...
        /// </returns>
        /// Marked as virtual for unit tests
        internal protected virtual long MillisecondsUntilDangerZoneEntered()
        {
            if (this.SessionMonitor == null)
            {
                throw new SessionMonitorNotSetException("SessionMonitor is not set.");
            ...
            return this.SessionMonitor.MillisecondsUntilLeaseEntersDangerZone(this.LookupTime);
        }
